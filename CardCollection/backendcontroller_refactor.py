...
class BackendController(QObject):
    ...
    saveResults = Signal(str) # Signal that emits JSON string
    updateResults = Signal(str)
    syncResults = Signal(str)
    ...
    @Slot(str, bool)
    def request_collection_update(self, card_id: str, is_add: bool):
        """Handle collection updates from any QML view"""
        update_thread = threading.Thread(
            target=self._perform_collection_update, 
            args=(card_id, is_add)
        )
        update_thread.start()

    def _perform_collection_update(self, card_id: str, is_add: bool):
        try:
            collection_handler = collectionhandler.CollectionHandler()
            if is_add:
                updated_collection = collection_handler.handle_update_collection(card_id)
            else:
                updated_collection = collection_handler.remove_entry_from_collection(card_id)
                
            json_result = json.dumps(updated_collection)
        except Exception as e:
            json_result = json.dumps({"error": str(e)})

        QMetaObject.invokeMethod(
            self, 
            "_emit_update_results",
            Qt.QueuedConnection,
            Q_ARG(str, json_result)
        )

    @Slot(str)
    def _emit_update_results(self, json_result):
        self.updateResults.emit(json_result)
        
    def request_remove_entry_from_collection(self, card_id: str):
        """
        Provide an interface for the front end to remove an entry from the user's collection.
        """
        try:
            # Remove entry from collection using CollectionHandler
            collection_handler = collectionhandler.CollectionHandler()
            collection_handler.remove_entry_from_collection(card_id)
            json_result = json.dumps({"status": "success", "message": "Entry removed from collection."})
            #print("DEBUG: Entry removed from collection")

        except Exception as e:
            json_result = json.dumps({"error": str(e)})
            #print("DEBUG: Exception occurred during entry removal:", e)

        # Emit the updateResults signal with confirmation or error
        self.updateResults.emit(json_result)

    def _perform_remove_entry_from_collection(self, card_id):
        try:
            # Remove entry from collection using CollectionHandler
            collection_handler = collectionhandler.CollectionHandler()
            collection_handler.remove_entry_from_collection(card_id)
        ...
        # Emit the updateResults signal with confirmation or error
        QMetaObject.invokeMethod(self, "_emit_update_results",
                                 Qt.QueuedConnection,
                                 Q_ARG(str, json_result))
